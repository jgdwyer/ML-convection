# Intended to be run on yellowstone: set the environment with the below commands
# module load python
...
for i, file_day in enumerate(file_days):
    ...
    zTin = f.variables['tg_before_convection'][:] # N_time x N_lev x N_lat x N_lon
    ...
    zPout_all = zPout + f.variables['condensation_rain'][:] # N_time x N_lat x N_lon
    lat = f.variables['lat'][:]
    f.close()
    # Note that because of the way the data is now stored, we no longer need to
    # do any sort of shifting. Temperatures and humidities are explicitly stored
    # before convection is called.
...
pickle.dump([Tin[:,:,randind_trn], qin[:,:,randind_trn], Tout[:,:,randind_trn],
             qout[:,:,randind_trn], Pout[:,randind_trn], lat],
                            open('./conv_training_v3.pkl', 'wb'))
pickle.dump([Tin[:,:,randind_tst], qin[:,:,randind_tst], Tout[:,:,randind_tst],
             qout[:,:,randind_tst], Pout[:,randind_tst], lat],
                            open('./conv_testing_v3.pkl', 'wb'))
# For convection + condensation learning
pickle.dump([Tin[:,:,randind_trn], qin[:,:,randind_trn], Tout_all[:,:,randind_trn],
             qout_all[:,:,randind_trn], Pout_all[:,randind_trn], lat],
                            open('./convcond_training_v3.pkl', 'wb'))
pickle.dump([Tin[:,:,randind_tst], qin[:,:,randind_tst], Tout_all[:,:,randind_tst],
             qout_all[:,:,randind_tst], Pout_all[:,randind_tst], lat],
